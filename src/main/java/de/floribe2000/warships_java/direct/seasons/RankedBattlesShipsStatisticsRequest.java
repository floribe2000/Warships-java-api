...
public class RankedBattlesShipsStatisticsRequest extends
	AbstractRequest<RankedBattlesShipsStatisticsRequest> implements IRequestAction<RankedBattlesShipsStatistics> {
	 ...
	 */
	@Override
	public RankedBattlesShipsStatistics fetch() {
		if (region == null) {
		...
		String url = baseUrl(region, path, language, getInstanceName()) + FieldType.SEASON_ID + seasons + FieldType.SHIP_ID + ships + FieldType.ACCOUNT_ID + accounts;
